...
namespace Simplecast.Client.Clients
{
    public class PlayerEmbedClient : IPlayerEmbedClient
    {
        ...
        {
            var apiResponse = await _restApiClient.GetApiResponseAsync<Embed>(UrlPathBuilder.GetPlayerEmbedUrl(podcastId, episodeId));
        ...
        {
            var apiResponse = await GetPlayerEmbedResponseAsync(podcastId, episodeId);
